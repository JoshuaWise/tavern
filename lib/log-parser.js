...
/**
 * @option header	-	If true, displays the file name at the top of output.
...
LogParser.prototype = {
	readFile: function (filename) {
		var title = '';
		if (this.__header) {title = makeSeperator(path.basename(filename), this.__width);}
...
function formatEntry(entry) {
	...
	var entryPlain = entry.replace(ansiRE, function (match, index) {
		ansis.push({index: index - ansisOffset, match: match});
	...
	while ((match = this.exec(entryPlain)) !== null) {
		...
		while (nextAnsi && nextAnsi.index <= endOfLine) {
			...
			line = line.slice(0, ansiPositionInLine) + nextAnsi.match + line.slice(ansiPositionInLine);
			ansisOffset += nextAnsi.match.length;
