...
module.exports = Command.extend({
	knownOptions: {'all': Boolean, 'clear': Boolean, 'flat': Boolean, 'boring': Boolean, 'output': Boolean},
	knownShorthands: {'a': '--all', 'c': '--clear', 'f': '--flat', 'b': '--boring', 'o': '--output'},
	maxRemainingArgs: 0,
	...
	execute: function (args, tavernDir) {
		...
		for (var i=filenames.length-1; i>=0; i--) {
			log += formatLogFile(filenames[i], this.options);
...
var formatLogFile = (function () {
	...
	function formatFlatLogEntry(entry) {
	...
	return function (filename, options) {
		var title = '';
		if (options.all) {title = clc.bold.inverse(makeSeperator(path.basename(filename)));}
		
		var contents = '' + fs.readFileSync(filename);
		if (options.boring) {contents = clc.strip(contents); title = clc.strip(title);}
		
		var entries = contents.split('\ufffc').filter(notEmpty);
		var indentLength = ~~entries.shift() || 8;
		if (options.flat) {
			return title + entries.map(formatFlatLogEntry).join('');
		} else {
			var context = new RegExp('.{1,' + ((process.stdout.columns || 80) - indentLength) + '}(\\n)?|(\\n)', 'g');
			context.indentLength = indentLength;
			return title + entries.map(formatLogEntry, context).join('');
		}
	};
}());
